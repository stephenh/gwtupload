 ...
 *         </ul>
 */
public class Uploader extends Composite implements IsUpdateable, IUploader, HasJsData, HasOnTokenRequestedHandlers {
  ...
  private Vector<IUploader.OnTokenRequestedHandler> onTokenRequestedHandlers = new Vector<IUploader.OnTokenRequestedHandler>();

  private static final String TAG_PERCENT = "percent";
  private static final String TAG_FINISHED = "finished";
  private static final String TAG_CANCELED = "cancelled";
  ...
  // Should pass the file token along with onFinished to be more async safe
  public Integer getFileToken() {
  ...
  public void update() {
    try {
      ...
      RequestBuilder reqBuilder = new RequestBuilder(RequestBuilder.GET, composeURL("status=" + fileToken, "c=" + requestsCounter++));
   ...
   */
  private void sendAjaxRequestToGetFileToken() {
    for (IUploader.OnTokenRequestedHandler handler : onTokenRequestedHandlers) {
      handler.onRequested();
    }
  }
  ...
  public HandlerRegistration addOnTokenRequestHandler(final OnTokenRequestedHandler handler) {
    assert handler != null;
    onTokenRequestedHandlers.add(handler);
    return new HandlerRegistration() {
      public void removeHandler() {
        onTokenRequestedHandlers.remove(handler);
      }
    };
  }
