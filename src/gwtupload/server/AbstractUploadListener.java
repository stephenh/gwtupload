...
public abstract class AbstractUploadListener implements ProgressListener, Serializable {
  ...
  static protected String className = AbstractUploadListener.class.getName().replaceAll("^.+\\.", "");
  ...
  public String toString() {
    return "total=" + getContentLength() + " done=" + getBytesRead() + " cancelled=" + isCanceled() + " saved=" + saved;
  }
  ...
  public void update(long done, long total, int item) {
    ...
    if (isCanceled()) {
      ...
      logger.info(className + " " + sessionId + " The upload has been canceled after " + 
            bytesRead + " bytes received, raising an exception (" + eName + 
            ") to close the socket");
